...
internal final class P45E1Tests: XCTestCase {

    let sut = P45E1.profile
    ...
    func testPathUpperLeft() {
        ...
        XCTAssertEqual((sut[0] as? ArcElement)?.radius, 307, accuracy: defaultAccuracy)
        XCTAssertEqual((sut[0] as? ArcElement)?.endPoint?.x, -21.542, accuracy: defaultAccuracy)

        XCTAssertEqual((sut[1] as? ArcElement)?.radius, 12.7, accuracy: defaultAccuracy)
        ...
        XCTAssertEqual((sut[6] as? ArcElement)?.center.y, -53.98, accuracy: defaultAccuracy)

        XCTAssertEqual((sut[7] as? ArcElement)?.radius, 508, accuracy: defaultAccuracy)
        XCTAssertEqual((sut[7] as? ArcElement)?.center, (-514.945, -71.44), accuracy: defaultAccuracy)
    }
    ...
    func testPathLowerLeft() {
        ...
        XCTAssertEqual((sut[13] as? LineElement)?.start.x, -63.5)
        XCTAssertEqual((sut[13] as? LineElement)?.end, (-63.5, -141.28))

        XCTAssertEqual((sut[14] as? ArcElement)?.radius, 1.6)

        XCTAssertEqual((sut[15] as? LineElement)?.start, (-61.9, -142.88))
        XCTAssertEqual((sut[15] as? LineElement)?.end, (61.9, -142.88))
    }
    ...
    func testPathLowerRight() {
        ...
        XCTAssertEqual((sut[17] as? LineElement)?.start, (63.5, -141.28))
        XCTAssertEqual((sut[17] as? LineElement)?.end.x, 63.5)
        ...
        XCTAssertEqual((sut[23] as? ArcElement)?.center, (514.945, -71.44), accuracy: defaultAccuracy)
    }
    ...
    func testPathUpperRight() {
        ...
        XCTAssertEqual((sut[24] as? ArcElement)?.center.y, -53.98)
        ...
        XCTAssertEqual((sut[29] as? ArcElement)?.radius, 12.7)

        XCTAssertEqual((sut[30] as? ArcElement)?.radius, 307)
        XCTAssertEqual((sut[30] as? ArcElement)?.endPoint, (0, 0), accuracy: defaultAccuracy)
        XCTAssertEqual((sut[30] as? ArcElement)?.startPoint.x, 21.542, accuracy: defaultAccuracy)
    }
}
