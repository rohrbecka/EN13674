...
internal final class P41E1Tests: XCTestCase {

    let sut = P41E1.profile
    ...
    func testPathUpperLeft() {
        ...
        XCTAssertEqual((sut[0] as? ArcElement)?.endPoint?.x, -24.1025, accuracy: defaultAccuracy)
    ...
    func testPathUpperRight() {
        ...
        XCTAssertEqual((sut[31] as? ArcElement)?.radius, 10)

        XCTAssertEqual((sut[32] as? ArcElement)?.radius, 400)
        XCTAssertEqual((sut[32] as? ArcElement)?.endPoint, (0, 0), accuracy: defaultAccuracy)
        XCTAssertEqual((sut[32] as? ArcElement)?.startPoint.x, 24.1025, accuracy: defaultAccuracy)
    }
}
