...
internal struct P50E4: RailProfile {
    ...
    public func profile(resolution: Double) -> [CGPoint] {
        let profile = Path {
            ...
            Fillet(radius: 8)
            // TODO: This concatenation of two arcs is not working
            Arc(radius: 22, centerY: -152+75.1+18.6, .clockwise)
            Arc(radius: 508, centerY: -152+75.1, .clockwise)
            // this is a workaround as an Arc with only a center can't be drawn, therefore it is split up by the Line
            Line(from: (-7.5, -152+75.1001), to: (-7.5, -152+75.0999))
            Arc(radius: 508, centerY: -152+75.1, .clockwise)
            Fillet(radius: 15)
            ...
            Arc(radius: 508, centerY: -152+75.1, .clockwise)
            // this is a workaround as an Arc with only a center can't be drawn, therefore it is split up by the Line
            Line(from: (7.5, -152+75.0999), to: (7.5, -152+75.1001))
            // TODO: This concatenation of two arcs is not working
            Arc(radius: 508, centerY: -152+75.1, .clockwise)
            Arc(radius: 22, centerY: -152+75.1+18.6, .clockwise)
            Fillet(radius: 8)
            ...
            Arc(radius: 300, center: (0, -300), fromX: 20.025/2.0, to: (0, 0), .counterclockwise)
        }
